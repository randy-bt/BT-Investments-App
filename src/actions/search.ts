...
export async function globalSearch(input: unknown): Promise<ActionResult<SearchResults>> {
  try {
    ...
    const leadsPromise = supabase
      .from('leads')
      .select('id, name, status, stage, closed, properties(address)')
    ...
    const leadPhonesPromise = supabase
      .from('lead_phones')
      .select('lead_id, leads!inner(id, name, status, stage, closed)')
    ...
    const leadMap = new Map<string, { id: string; name: string; status: string; stage: string; closed: boolean; address?: string }>()
    for (const lead of leads.data ?? []) {
      const props = (lead as Record<string, unknown>).properties as { address: string }[] | undefined
      const address = props?.[0]?.address
      leadMap.set(lead.id, { id: lead.id, name: lead.name, status: lead.status, stage: lead.stage, closed: lead.closed ?? false, address })
    }
    for (const row of [...(leadPhones.data ?? []), ...(leadEmails.data ?? [])]) {
      const lead = row.leads as unknown as { id: string; name: string; status: string; stage: string; closed: boolean }
      if (lead && !leadMap.has(lead.id)) leadMap.set(lead.id, { ...lead, closed: lead.closed ?? false })
    }
