...
export async function POST(req: NextRequest) {
  ...
  // Fallback cutoff for any mailbox without its own. Per-account watermarks
  // and cutoffs are read inside the account loop.
  const cutoffStr = await readSetting(supabase, CUTOFF_KEY)
  const globalCutoff = cutoffStr ? new Date(cutoffStr) : new Date(Date.now() - 7 * 864e5)
  ...
  // Every configured mailbox (Randy 7/31). JV senders reach more than one
  // inbox, so scanning a single account silently dropped whatever landed in
  // the others. Each account carries its own watermark and cutoff.
  const accounts = getJvAccounts()
  if (accounts.length === 0) {
    return NextResponse.json({ ok: false, error: 'No JV mailbox configured' }, { status: 500 })
  }
  ...
  const perAccount: Record<string, { processed: number; created: number; skipped: number }> = {}
  ...
  try {
    ...
    // continues from there. The budget is shared across mailboxes, since the
    // 300s ceiling is per request, not per account.
    ...
    for (const account of accounts) {
      ...
      // Per-account watermark and cutoff. The first mailbox keeps the
      // unsuffixed keys so it does not rescan its whole history.
      const uidKey = `${LAST_UID_KEY}${account.suffix}`
      const sinceUid = parseInt((await readSetting(supabase, uidKey)) ?? '0', 10) || 0
      const accountCutoff = await readSetting(supabase, `${CUTOFF_KEY}${account.suffix}`)
      const sinceDate = accountCutoff ? new Date(accountCutoff) : globalCutoff

      const fetched = await fetchNewJvMessages(account, { sinceUid, sinceDate })
      const ordered = [...fetched.messages].sort((a, b) => a.uid - b.uid)
      const messages = ordered.slice(0, budget)
      const accountTruncated = ordered.length > messages.length
      if (accountTruncated) truncated = true
      const maxUid = accountTruncated ? messages[messages.length - 1].uid : fetched.maxUid
      budget -= messages.length

      const before = { created, skipped }
      processed += messages.length
      let accountExtractionFailed = false
      ...
      for (const m of messages) {
        ...
        try {
          ...
          // Stop this mailbox so the failed message is retried next run, but
          // let the other mailboxes finish their own batches.
          accountExtractionFailed = true
      ...
      // Advance this mailbox's watermark only when its own extraction ran
      // clean, so a failed message is retried rather than skipped past.
      if (!accountExtractionFailed && maxUid > sinceUid) {
        await supabase
          .from('app_settings')
          .upsert({ key: uidKey, value: String(maxUid) }, { onConflict: 'key' })
      }

      // Mark processed emails read in this mailbox (best-effort; a flag
      // failure must never fail the scan).
      try {
        await markJvMessagesSeen(account, processedUids)
      } catch (e) {
        console.error(`[jv/scan] mark-seen failed for ${account.user}:`, (e as Error).message)
      }

      perAccount[account.user] = {
        processed: messages.length,
    ...
    return NextResponse.json({
      ok: true, processed, created, skipped, extractionFailed, truncated,
      unlistedSenders, accounts: perAccount,
    })
  } catch (e) {
